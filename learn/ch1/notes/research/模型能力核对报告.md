...
- **引用**：[chapter1/README.md#L25-L31](../../../../../chapter1/README.md#L25-L31)（1-1 context、1-2 web-search-agent、1-3 search-codegen、1-4 image-gen-workflow、7-1&7-2 learning-from-experience）
...
- **引用**：[context/main.py#L477](../../../../../chapter1/context/main.py#L477)（png）、[#L881](../../../../../chapter1/context/main.py#L881)（md）、[#L826](../../../../../chapter1/context/main.py#L826)（json 默认名）、[#L812](../../../../../chapter1/context/main.py#L812)（ablation 入口带 provider 参数）
...
- **引用**：
  - [web-search-agent/README.md#L119](../../../../../chapter1/web-search-agent/README.md#L119)：`--provider` 仅 `kimi` / `offline-demo` 两值；[#L127-L131](../../../../../chapter1/web-search-agent/README.md#L127-L131) offline 命令；[#L142-L143](../../../../../chapter1/web-search-agent/README.md#L142-L143) 单问题与 `--max-steps 3 --output result.json`
  - [agent.py#L156](../../../../../chapter1/web-search-agent/agent.py#L156)：`self.formula_uri = "moonshot/web-search:latest"`；[#L238-L239](../../../../../chapter1/web-search-agent/agent.py#L238-L239)：调用 `/formulas/.../fibers` —— **搜索执行绑定 Moonshot 端点**
  - [README.md#L279](../../../../../chapter1/web-search-agent/README.md#L279)：kimi-k3 以 `reasoning_effort=max` 运行，单次 1~数分钟，`SEARCH_TIMEOUT=180`（[#L244](../../../../../chapter1/web-search-agent/README.md#L244)）
  - [env.example#L12-L14](../../../../../chapter1/web-search-agent/env.example#L12-L14)：OpenRouter 兜底**无实时搜索**，不能用于 1-2 验收
...
### A4 ✅ 1-3 命令与验收点
- **引用**：[search-codegen/README.md#L124-L137](../../../../../chapter1/search-codegen/README.md#L124-L137)（openai 正式命令、dashscope 命令、`--dry-run` 检查请求结构）；[#L51-L54](../../../../../chapter1/search-codegen/README.md#L51-L54)（验收须有 `web_search_call` + `code_interpreter_call` 回执与 URL 引用）；[#L77-L79](../../../../../chapter1/search-codegen/README.md#L77-L79)（东盟答案：吉隆坡—新加坡 **316.35 km**）；[#L8](../../../../../chapter1/search-codegen/README.md#L8)（百炼 qwen3.7-plus 是唯一实测验收后端）
- **验证**：`python main.py --backend openai --dry-run --request "test"` 不发请求即打印请求体

### A5 ✅（含 2 处修正）1-4 配置与运行
- **已证实**：
  - 命令：`pip install -r requirements.txt`（google-genai openai requests python-dotenv）、`python main.py --route workflow`、`--requirement windowsill-plant`、`python -m pytest` —— [image-gen-workflow/README.md#L70-L87](../../../../../chapter1/image-gen-workflow/README.md#L70-L87)
  - 架构：改写节点 kimi-k3（**OpenAI 兼容接口**）+ 通义万相 wan2.2-t2i-flash（DashScope 国际站，**异步任务接口**，有轮询参数）—— [README.md#L31-L34](../../../../../chapter1/image-gen-workflow/README.md#L31-L34)、[config.py#L27-L37](../../../../../chapter1/image-gen-workflow/config.py#L27-L37)、[#L60-L61](../../../../../chapter1/image-gen-workflow/config.py#L60-L61)
  - 海报文案被丢进 negative_prompt 的结论 —— [README.md#L146-L149](../../../../../chapter1/image-gen-workflow/README.md#L146-L149)
- **⚠️ 修正 1（我此前遗漏的坑）**：[config.py#L63-L65](../../../../../chapter1/image-gen-workflow/config.py#L63-L65) 的 `required_env` 含 **GEMINI_API_KEY 和 OPENAI_API_KEY**，且 [main.py#L149](../../../../../chapter1/image-gen-workflow/main.py#L149) 调用 `Config.validate()` → **即使只跑 workflow 路线，4 个变量也必须非空**（可填占位符）
- **⚠️ 修正 2（比我之前说的更简单）**：改写节点换 GLM **不用改任何代码**，`.env` 三行即可（因为 [config.py#L27-L29](../../../../../chapter1/image-gen-workflow/config.py#L27-L29) 就是 OpenAI 兼容配置）：
  ```env
  KIMI_API_KEY=<你的ZHIPU_API_KEY>
  KIMI_BASE_URL=https://open.bigmodel.cn/api/paas/v4
  REWRITE_MODEL=glm-5.3-flash
  ```
  风险点：pipeline 会显式传 temperature（历史失败记录 [README.md#L118-L119](../../../../../chapter1/image-gen-workflow/README.md#L118-L119)：kimi-k3 拒绝 temperature=0.3），GLM 应接受，跑 1 句需求即验（E5）

### A6 ✅（含 1 处证伪）7-1 / 7-2 命令与 provider 支持
- **已证实**：
  - 7-1 离线：`python experiment.py --mode qlearning --rl-episodes 10000 --seed 42`（约 3 秒）—— [README.md#L168-L174](../../../../../chapter1/learning-from-experience/README.md#L168-L174)
  - 对比：`python experiment.py --mode both --model kimi-k3`；演示：`python quick_demo.py` —— [README.md#L134-L180](../../../../../chapter1/learning-from-experience/README.md#L134-L180)
  - 官方 Kimi K3 首局实测 416.11 秒 / 17 次串行调用 —— [README.md#L204-L207](../../../../../chapter1/learning-from-experience/README.md#L204-L207)
  - provider 支持：`LLM_PROVIDER` 仅 **moonshot（默认）/ dashscope / qwen / bailian** + OpenRouter 兜底 —— [env.example#L1-L2](../../../../../chapter1/learning-from-experience/env.example#L1-L2)、[llm_agent.py#L86-L106](../../../../../chapter1/learning-from-experience/llm_agent.py#L86-L106)（dashscope 分支走共享 registry，其余硬编码 moonshot）→ **zhipu/deepseek/mimo/agnes 跑 7-2 需在 `__init__` 加分支** ✅
  - 利好：[llm_agent.py#L19-L24](../../../../../chapter1/learning-from-experience/llm_agent.py#L19-L24) 温度适配函数注释明确提到 Doubao/DeepSeek —— 作者已预留非 reasoning 模型兼容
- **❌ 已证伪（我的错误）**：README [#L187](../../../../../chapter1/learning-from-experience/README.md#L187) 写 `python run_experiment_7_2.py`，但目录里实际文件是 **`run_experiment_8_2.py`**（Glob 证据，另有 `finalize_experiment_8_2.py`）。**以实际文件为准：`python run_experiment_8_2.py`**。这是仓库 README 与文件名的 drift（README 还称本目录为实验 7-1/7-2，文件名却用 8_2，双重编号并存）

### A7 ✅ DashScope 默认是国际站 URL
- **引用**：[search-codegen/env.example#L18-L21](../../../../../chapter1/search-codegen/env.example#L18-L21)（`dashscope-intl...`，注释给出中国区地址）、[image-gen-workflow/env.example#L25](../../../../../chapter1/image-gen-workflow/env.example#L25)、[registry.py#L26-L36](../../../../../agentbook/providers/registry.py#L26-L36)（共享 registry 默认**中国区**）
- **验证**：直接打开三个文件对照
...
- **引用**：[registry.py#L66-L71](../../../../../agentbook/providers/registry.py#L66-L71)（zhipu→`glm-5.3`，读 ZHIPU_API_KEY）、[#L57-L65](../../../../../agentbook/providers/registry.py#L57-L65)（deepseek→`deepseek-v4-flash`，读 DEEPSEEK_API_KEY）
- **验证**：1-1 目录下 `python main.py --provider deepseek` 直接可跑

### B2 ✅ registry 无 agnes / mimo 条目；doubao 只挂 LLM
- **引用**：[registry.py#L25-L121](../../../../../agentbook/providers/registry.py#L25-L121)（全表 12 个 provider）；[#L134-L142](../../../../../agentbook/providers/registry.py#L134-L142) 注释确认**新增一条 entry 即被各章 `--provider` 自动选中**，无需改 argparse
...
- **接入 1-2 的要点**：与 Moonshot 的 Formula（tools→fibers 两段式）不同，MiMo 的搜索是**服务端内联**（一次 chat/completions 返回带引用的答案）。改造点在 [agent.py](../../../../../chapter1/web-search-agent/agent.py) 的搜索实现层（README 标注的 `search_impl()` 扩展点即为此设计）；ReAct 轨迹的"observe"步骤需从 annotations 解析。响应形态先按 E4 实测确认
...
- **引用**：[registry.py#L69](../../../../../agentbook/providers/registry.py#L69) 默认模型即 `glm-5.3`；[bigmodel.cn 官网](https://www.bigmodel.cn/)（"GLM-5.3 Latest Flagship Model"）；[发布记录](https://docs.bigmodel.cn/cn/update/new-releases)（2026-08-19 GLM-5.3 上线，1M 上下文）
...
| N3 | ⚠️ **修正（用户质疑后逐行复核）**：1-3 代码**不锁模型也不锁平台**——`--backend dashscope` 只是第二配置槽（[config.py#L63-L64](../../../../../chapter1/search-codegen/config.py#L63-L64)，key/base_url/model 全来自 env），`--model` 可直接覆盖（main.py 的 `Config.resolve(args.backend, args.model)`），模型身份检查是"请求==返回"的相对检查（[run_experiment_1_3.py#L120-L125](../../../../../chapter1/search-codegen/run_experiment_1_3.py#L120-L125)），后端白名单按 flag 名记录（#L51）。真正的硬门槛只有一条：响应含**服务端代跑**的 `web_search_call`/`code_interpreter_call` 回执（#L137-L140；README#L51-L54、L95-L97）——这是平台能力非模型能力；**智谱 `/v1/responses` 是否代跑未验证，T6 探针定夺**：通过则 1-3 全程 flash+智谱，不通才退回百炼 | [agent.py#L65-L76](../../../../../chapter1/search-codegen/agent.py#L65-L76)（provider 由 base_url 推导；指向智谱时用 OpenAI 风格工具声明）、[README#L95-L97](../../../../../chapter1/search-codegen/README.md#L95-L97) |
| N4 | ✅ **1-1 支持 `--model` 且解析层显式优先**：[context/main.py#L1215-L1219](../../../../../chapter1/context/main.py#L1215-L1219) 定义 `--model`；[resolution.py#L167-L169](../../../../../agentbook/providers/resolution.py#L167-L169) `if model_clean: resolved_model = model_clean` → 1-1 换 flash **零代码改动**，B2 的 registry 建议降级为可选 | 本地文件重读 |
