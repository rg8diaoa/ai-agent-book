...
את המימוש ההנדסי המינימלי של סוכן מודרני אפשר לבטא בנוסחה תמציתית אחת: **סוכן = LLM (מודל שפה גדול, ‏Large Language Model) + הקשר + כלים**. סימני החיבור כאן מציינים צירוף של רכיבים הנדסיים, ולא הגדרה פורמלית מתחום הלמידה בחיזוקים; וחשוב מכך, הנוסחה מתארת רק את המימוש שבתוך גבול הסוכן, ו**אינה כוללת את ה‑Environment (הסביבה) שעמה הסוכן מקיים אינטראקציה**. כל מונח בה יש להבין במובן רחב, אך עם גבול ברור:
...
- **הגדרות כלים (Tool Definitions)**: מצהירות על השמות, התיאורים התפקודיים ופורמטי הפרמטרים של הכלים העומדים לרשות הסוכן. ללא הגדרות כלים, הסוכן אינו יכול לזהות או לקרוא לשום כלי — אך גם אינו משתתק בשל כך; מחקר ביטול (ניסוי 1‑1) יראה מה הוא עושה במקום זאת. הגדרות כלים, יחד עם הנחיית המערכת, מהוות את **הקידומת הסטטית** הנותרת ללא שינוי לאורך השיחה. (זו התבנית היסודית; מאז 2026, מסגרות ייצור יכולות גם לטעון סכמות כלים מלאות לפי דרישה בסוף ההקשר מבלי לשבור את הקידומת — ראו את חלק הגדרות הכלים בפרק 2 ובפרק 4.)
...
כדי לוודא שכל רכיב באמת הכרחי, הדרך הישירה ביותר היא **מחקר אבלציה** (Ablation Study): בדומה לרופא המוציא סיבות אחת אחת בעת אבחון — מסירים את רכיב A ובודקים אם המערכת עדיין פועלת כשורה, אחר כך מסירים את רכיב B, וכן הלאה, וכך שופטים את תרומתו של כל רכיב. ניסוי 1-1 בדיוק לפי קו המחשבה הזה בחן באופן שיטתי את חמשת הרכיבים שלעיל.
...
>
> תוצאות הניסוי חשפו את תפקידו של כל רכיב הקשר — וגם שאין הם חשובים במידה שווה. **הגדרות כלים** (חלק מהקידומת הסטטית) הן הבסיס ליכולת הפעולה של הסוכן; בלעדיהן, הסוכן אינו יכול לקרוא לשום כלי. אלא שאיבוד יכולת הפעולה אינו שקול לשתיקה: המודל עדיין מחזיר תשובה מעוצבת היטב ובנימה נחרצת, אך המספרים שבה מגיעים מהזיכרון הפרמטרי ולא מתצפיות, והם מוצגים בדיוק כמו תשובה שנגזרה באמת מפלט הכלים. אם יסרב בגלוי או ימציא במקום — הדבר תלוי בעיקר בשיעור ההזיות ובמידת היושרה של המודל עצמו; מגבלה בהנחיה מסוג "אל תעריך את שערי החליפין בעצמך" מפחיתה את הסיכוי להמצאה אך אינה מבטלת אותו. **תוצאות כלים** הן המפתח לבקרת מעגל סגור; בלעדיהן הסוכן פועל "בעיוורון" ומנסה שוב ושוב עד שתקציב האיטרציות שלו נגמר. **תהליך ההיסק** (חלק ה‑reasoning בהודעות העוזר) מתעד *מדוע* נעשה צעד, ואילו תוצאות הכלים מתעדות *מה קרה*; כאשר אפשר לשחזר את הראשון מתוך השני, ויתור עליו בהיסטוריה כמעט אינו עולה דבר. **היסטוריית הודעות** (הודעות משתמש, הודעות עוזר ותוצאות כלים מסבבים קודמים) מונעת פעולות מיותרות ומונעת חזרה על אותן טעויות.
>
> התובנה המרכזית של הניסוי: **ההקשר קובע מה הסוכן יכול לראות, והסוכן יכול להחליט רק על בסיס מה שהוא רואה**. אלא שהרכיבים אינם שקולים; אמת המידה היא אם המידע שרכיב נושא ניתן לשחזור ממקום אחר — טענות מסוג "אין רכיב שאפשר לוותר עליו" יש למדוד ולא להניח, והמודלים מתחלפים מהר דיו כדי שאותו ניסוי ביטול על מודל חדש יותר יגיע למסקנה שונה לגמרי. יש נקודה נוספת, חשובה עוד יותר בפרקטיקה ההנדסית: **"נתן תשובה" אינו "השלים את המשימה"**. כשחסר רכיב הקשר, הכישלון האופייני אינו יציאה עם שגיאה אלא תשובה שנראית ללא רבב.
...
נתחיל בשלד ההרצה המינימלי. הוא מראה **כיצד פועל המנגנון**: ה‑Model רק מחליט מה הצעד הבא, ה‑Harness מרכיב את ההקשר ומאמת ומריץ את הכלים, וה‑Environment מייצר את שינויי המצב האמיתיים ואת התצפיות. בהמשך הספר נמשיך להשתמש בפסאודו‑קוד בסגנון Python; הפסאודו‑קוד אינו רץ כמות שהוא ואינו מתאים ל‑SDK מסוים. הקוד הרץ הקונקרטי נמצא במאגר הקוד הנלווה לספר.
...
בהתבסס על ניסיונה של Anthropic, מערכות סוכן מוצלחות פועלות לפי שלושה עקרונות ליבה[^ch1-anthropic-building-effective-agents].
...
[^ch1-anthropic-building-effective-agents]: Anthropic. "Building effective agents", דצמבר 2024. https://www.anthropic.com/engineering/building-effective-agents
...
דפוסי תזמור הם האופן שבו ה‑Harness מארגן את שכבת "ההקשר והכלים" שלו — הם קובעים כיצד ההקשר זורם בין קריאות LLM, כיצד כלים מתוזמנים, והאם נתיב הביצוע של הסוכן קבוע מראש או נוצר דינמית. תזמור סוכנים התפתח מפשוט למורכב, ולכל דפוס יש מקרי שימוש מתאימים ופשרות. מניסיונה של Anthropic בעבודה עם עשרות צוותים הבונים סוכני LLM[^ch1-anthropic-building-effective-agents], המימושים המוצלחים ביותר משתמשים לעיתים רחוקות במסגרות מורכבות; הם משתמשים בדפוסים פשוטים וברי‑הרכבה.

בעת בניית יישום LLM, פעלו לפי העיקרון של מעבר מפשוט למורכב. התחילו בבחינת קריאת LLM יחידה. אם פרומפטים טובים יותר ודוגמאות בתוך ההקשר יכולים לפתור את הבעיה, אל תכניסו מערכת סוכנים. כאשר נדרש עיבוד רב־שלבי, שקלו תהליך עבודה בתרחישים שאפשר לפרק בבירור לתת־משימות קבועות. השתמשו בסוכן אוטונומי רק כשנדרשות החלטות דינמיות ודרכי ביצוע גמישות. זכרו שמערכות סוכנים בדרך כלל מחליפות זמן השהיה ועלות בביצועי משימה טובים יותר, ולכן יש לשקול היטב אם ההחלפה כדאית.

דוגמה נגדית נפוצה היא לבנות כבר בתחילת הדרך תהליך עבודה מורכב מאוד או מערכת מרובת סוכנים. למשל, כאשר מתכננים סוכן ש״מחלץ זיכרונות אישיים ממיליון הודעות צ׳אט״, מודלים מסוימים של בינה מלאכותית משרטטים במהירות צינור עבודה שנראה קפדני: תחילה מחלקים את השיחות למקטעים, אחר כך מציבים ברצף סוכנים לחילוץ, לאימות ראיות, ליישוב זהויות, לארגון זיכרונות ולבדיקת המיזוג, ולבסוף בונים גרף עובדות, יומן כיסוי וגרסאות בלתי־משתנות. כל רכיב נשמע הגיוני בפני עצמו, אך יחד הם יוצרים מערכת בלתי יעילה ובלתי אמינה מאוד. מכיוון שטופולוגיית הביצוע של תהליך עבודה מורכב קבועה, כל חריגה חדשה מובילה בקלות להוספת צומת נוסף: הארכיטקטורה נעשית מורכבת יותר ויותר, אך כללית פחות ופחות. הכרעות סמנטיות שהמודל היה יכול לקבל מתוך ההקשר מתקבעות במקום זאת בתהליך העבודה.

לכן, **חשיבותו של ה‑Harness אינה אומרת שככל שהוא מורכב יותר כך הוא טוב יותר**. אתר Manus מסכם את הפשרה הזאת במילים “Less structure, more intelligence.”[^ch1-manus-less-structure] תחילה יש לתת לסוכן בעל יכולת מספקת יעד ברור, את ההקשר הנחוץ וכלים בני־הרכבה, ולהסביר לו בשפה טבעית כיצד לבדוק, להשוות ולטפל בהתנגשויות כפי שאדם היה עושה. על התוכנית לקבע רק גבולות שחייבים להתקיים תמיד, כגון הרשאות, איסור לדרוס את חומרי המקור ופרסום אטומי. רק כאשר האילוצים העסקיים עצמם מחייבים זאת, או כאשר הערכות חושפות שוב ושוב דפוס כשל יציב, יש להפוך את השלב המתאים למאמת ייעודי, לסוכן עצמאי או לתהליך דטרמיניסטי. מבנה טוב אינו מתרגל מראש את כל החשיבה עבור הסוכן; הוא שומר על הגבולות ומחזיר למודל את מרחב ההחלטה שבתוכם.
...
יש עוד דרך לשלב בין השניים: **תחילה הסוכן האוטונומי כותב את תהליך העבודה, ולאחר מכן תהליך העבודה הוא שמבצע**. לאחר שהסוכן קורא את המשימה הוא מחליט בעצמו על הטופולוגיה ומייצר קטע קוד של תזמור; ברגע שהקוד נוצר, שלב הביצוע חוזר לדטרמיניזם של תהליך העבודה. כך נשמרת גמישותו של הסוכן האוטונומי מול משימות לא מוכרות, ואין צורך שהמודל יכריע בכל תזמון ותזמון. פרק 10 דן בדפוס זה בהרחבה.
...
**ההקשר הוא הגורם המכריע**: ההקשר מורכב מקידומת סטטית (הנחיית מערכת + הגדרות כלים) וממסלול דינמי (היסטוריית הודעות). ניסויי ביטול מראים שהרכיבים אינם שקולים: הסרת הגדרות הכלים או תוצאות הכלים גוזלת מיד את יכולת הפעולה או את סגירת הלולאה, ואילו מחיר הסרתם של שני האחרים תלוי בשאלה אם אפשר לשחזר את המידע הזה מן התצפיות הנוכחיות. מהותה של לולאת ReAct היא הוספה למסלול, שוב ושוב, כך שהמודל ממשיך לקדם את המשימה.
...
4. ★★ בניסוי הביטול, היעדר "משוב תוצאות הכלים" גורם לסוכן לנסות שוב ושוב עד שתקציב האיטרציות שלו נגמר. בסביבת ייצור, מלבד תוצאות כלים חסרות, אילו מצבים נוספים עלולים להכניס סוכן ללולאה כזו? אילו מנגנוני זיהוי וסיום הייתם מעצבים?
