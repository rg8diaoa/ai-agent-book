...
현대 Agent의 최소 공학적 구현은 간결한 공식 하나로 표현할 수 있습니다. **Agent = LLM(대규모 언어 모델, Large Language Model) + 컨텍스트 + 도구**. 여기서 더하기 기호는 공학적 구성 요소의 조합을 뜻하며, 강화학습에서의 형식적 정의가 아닙니다. 더 중요한 것은 이 공식이 Agent 경계 안쪽의 구현만을 서술할 뿐 **Agent가 상호작용하는 Environment(환경)를 포함하지 않는다**는 점입니다. 각 낱말은 넓게, 그러나 경계가 분명하게 이해해야 합니다.
...
- **도구 정의(Tool Definitions)**: 에이전트가 사용할 수 있는 도구의 이름, 기능 설명, 파라미터 형식을 선언합니다. 도구 정의가 없으면 에이전트는 어떤 도구도 인식하거나 호출할 수 없습니다. 다만 그렇다고 해서 멈추지는 않습니다. 실험 1-1의 제거 실험에서 이를 살펴봅니다. 도구 정의는 시스템 프롬프트와 함께 대화 내내 바뀌지 않는 **정적 접두부**를 이룹니다. 이것이 기본 패턴이지만, 2026년 이후의 상용 프레임워크는 접두부를 깨뜨리지 않으면서 전체 도구 스키마를 필요할 때 컨텍스트 끝에 동적으로 불러올 수도 있습니다. 2장의 도구 정의 절과 4장을 참조하세요.
...
각 구성 요소가 정말로 없어서는 안 되는지 검증하는 가장 직접적인 방법은 **절제 실험**(Ablation Study)입니다. 의사가 진단할 때 원인을 하나씩 배제하듯, 먼저 A 구성 요소를 빼고 시스템이 여전히 정상인지 보고, 다음으로 B 구성 요소를 빼고, 이런 식으로 각 구성 요소의 기여를 판단합니다. 실험 1-1이 바로 이 방식으로 위의 다섯 구성 요소를 체계적으로 테스트했습니다.
...
>
> 실험 결과는 각 컨텍스트 요소의 역할과 함께, 그 요소들이 똑같이 중요하지는 않다는 사실도 보여 줬습니다. 정적 접두부의 일부인 **도구 정의**는 에이전트 행동 능력의 토대입니다. 이것이 없으면 어떤 도구도 호출할 수 없습니다. 다만 행동 능력을 잃는 것이 곧 침묵을 뜻하지는 않습니다. 모델은 여전히 형식이 잘 갖춰지고 단정적인 어조의 답을 내놓는데, 그 수치는 관측이 아니라 파라미터에 저장된 기억에서 나온 것이며, 실제로 관측에 근거해 얻은 답과 겉모습이 똑같습니다. 솔직하게 거절할지 그 자리에서 지어낼지는 주로 모델 자체의 환각률과 정직성에 달려 있습니다. 프롬프트에 “환율을 직접 추정하지 말라” 같은 제약을 넣어도 지어낼 확률을 낮출 뿐, 없애지는 못합니다. **도구 결과**는 폐루프 제어의 핵심입니다. 이것이 없으면 에이전트는 “눈먼” 채로 실행하며 반복 예산을 다 쓸 때까지 재시도를 되풀이합니다. 어시스턴트 메시지의 `reasoning` 부분인 **사고 과정**이 기록하는 것은 “왜 그렇게 했는가”이고, 도구 결과가 기록하는 것은 “무슨 일이 있었는가”입니다. 전자를 후자에서 다시 끌어낼 수 있을 때는 그것을 기록에서 덜어내도 대가가 거의 없습니다. 이전 라운드의 사용자 메시지, 어시스턴트 메시지, 도구 결과를 포함한 **메시지 기록**은 불필요한 반복을 막고 같은 실수를 되풀이하지 않게 합니다.
>
> 이 실험의 핵심 통찰은 다음과 같습니다. **컨텍스트는 의사결정 시점에 에이전트가 어떤 정보를 갖고 있는지를 결정하며, 에이전트는 그 정보만으로 결정할 수 있습니다.** 다만 요소들이 대등하지는 않습니다. 기준은 그 요소가 담은 정보를 다른 데서 다시 만들어 낼 수 있는가입니다. “어떤 요소도 빠질 수 없다” 같은 주장은 짐작이 아니라 실측으로 확인해야 하며, 모델이 워낙 빠르게 바뀌는 만큼 같은 제거 실험을 최신 모델에서 돌리면 다른 결론이 나올 수도 있습니다. 엔지니어링 실무에서 더 중요한 점이 하나 더 있습니다. **“답을 내놓았다”는 것이 “작업을 끝냈다”는 뜻은 아닙니다.** 컨텍스트 요소가 빠졌을 때의 전형적인 실패는 오류로 멈추는 것이 아니라, 흠잡을 데 없어 보이는 답을 내놓는 것입니다.
...
먼저 최소 실행 골격을 봅시다. 이것이 보여 주는 것은 **메커니즘이 어떻게 돌아가는가**입니다. Model은 다음 한 수를 정할 뿐이고, Harness는 컨텍스트를 조립하고 도구를 검증·실행하며, Environment는 실제 상태 변화와 관측을 만들어 냅니다. 이 책은 이후에도 Python 스타일 의사 코드를 씁니다. 의사 코드는 그대로 실행할 수 없고 특정 SDK에 대응하지도 않습니다. 구체적인 실행 가능 코드는 이 책의 부속 코드 저장소에 있습니다.
...
Anthropic의 경험에 따르면 성공적인 에이전트 시스템은 세 가지 핵심 원칙을 따릅니다[^ch1-anthropic-building-effective-agents].
...
[^ch1-anthropic-building-effective-agents]: Anthropic. "Building effective agents", 2024년 12월. https://www.anthropic.com/engineering/building-effective-agents
...
오케스트레이션 패턴은 하네스의 “컨텍스트와 도구” 계층을 구성하는 방식입니다. LLM 호출 사이에서 컨텍스트가 어떻게 흐르는지, 도구를 어떻게 예약하는지, 에이전트의 실행 경로가 미리 정해지는지 동적으로 생성되는지를 결정합니다. 에이전트 오케스트레이션은 단순한 방식에서 복잡한 방식으로 발전했으며, 패턴마다 알맞은 사용 사례와 상충 관계가 있습니다. Anthropic이 LLM 에이전트를 구축하는 수십 개 팀과 협업한 경험에 따르면[^ch1-anthropic-building-effective-agents] 가장 성공적인 구현은 복잡한 프레임워크보다 단순하고 조합 가능한 패턴을 사용합니다.

LLM 애플리케이션을 만들 때는 단순한 것에서 복잡한 것으로 나아간다는 원칙을 따르세요. 먼저 LLM 호출 하나를 고려합니다. 더 좋은 프롬프트와 컨텍스트 안 예시로 문제가 풀리면 에이전트 시스템을 도입하지 않습니다. 여러 단계가 필요하다면 고정된 하위 작업으로 명확히 나눌 수 있는 경우에 워크플로를 고려합니다. 동적 의사결정과 유연한 실행 경로가 필요할 때만 자율 에이전트를 사용합니다. 에이전트 시스템은 일반적으로 더 나은 작업 성능을 위해 지연과 비용을 맞바꾼다는 점을 기억하고, 그 대가가 가치 있는지 신중하게 평가해야 합니다.

흔한 반면교사는 처음부터 매우 복잡한 워크플로나 멀티 에이전트 시스템을 구축하는 것입니다. 예를 들어 “백만 건의 채팅 메시지에서 개인의 기억을 추출하는” 에이전트를 설계하라고 하면, 일부 AI 모델은 금세 겉보기에는 엄밀한 파이프라인을 그려 냅니다. 먼저 대화를 구간으로 나누고, 추출·증거 검증·신원 해석·기억 정리·병합 검토 에이전트를 차례로 배치한 뒤, 마지막에 사실 그래프·커버리지 원장·불변 버전을 구축합니다. 각 구성 요소는 따로 보면 타당해 보이지만, 합치면 매우 비효율적이고 신뢰하기 어려운 시스템이 됩니다. 복잡한 워크플로는 실행 토폴로지가 고정되어 있어 새로운 예외를 만날 때마다 노드를 더하기 쉽습니다. 아키텍처는 갈수록 복잡해지고 범용성은 오히려 떨어집니다. 원래 모델이 컨텍스트를 바탕으로 처리할 수 있었던 의미 판단까지 워크플로에 하드코딩됩니다.

따라서 **하네스가 중요하다는 말은 하네스가 복잡할수록 좋다는 뜻이 아닙니다**. Manus 공식 웹사이트는 이 선택을 “Less structure, more intelligence.”라고 요약합니다.[^ch1-manus-less-structure] 먼저 충분한 능력을 갖춘 에이전트에게 명확한 목표, 필요한 컨텍스트, 조합 가능한 도구를 제공하고, 사람처럼 검증하고 비교하며 충돌을 처리하는 방법을 자연어로 알려 줍니다. 프로그램에는 권한, 원본 자료를 덮어쓰지 않는 규칙, 원자적 게시처럼 언제나 지켜져야 하는 경계만 고정합니다. 비즈니스 제약 자체가 요구하거나 평가에서 특정한 안정적 실패 유형이 반복해서 드러날 때만 해당 단계를 전용 검증기, 독립 에이전트, 결정적 프로세스로 승격합니다. 좋은 구조는 에이전트의 모든 사고를 미리 대신하지 않습니다. 경계를 지키고, 그 안의 의사결정 공간을 모델에 돌려줍니다.
...
두 방식을 섞는 또 하나의 방법이 있습니다. **자율 에이전트가 먼저 워크플로를 써내고, 그 워크플로가 실행을 맡는 것**입니다. 에이전트는 과제를 파악한 뒤 스스로 토폴로지를 정해 오케스트레이션 코드를 생성합니다. 코드가 만들어지고 나면 실행 단계는 워크플로의 결정성으로 돌아갑니다. 이렇게 하면 처음 보는 과제 앞에서 자율 에이전트의 유연성은 그대로 두면서, 매번의 스케줄링마다 모델이 판단하지 않아도 됩니다. 이 형태는 10장에서 자세히 다룹니다.
...
**컨텍스트가 의사결정을 좌우합니다**: 컨텍스트는 정적 접두부(시스템 프롬프트 + 도구 정의)와 동적 궤적(메시지 기록)으로 구성됩니다. 제거 실험이 보여 준 것은 요소들이 대등하지 않다는 사실입니다. 도구 정의나 도구 결과를 없애면 행동 능력이나 폐루프 능력 자체가 사라지지만, 나머지 둘을 없앨 때의 대가는 그 정보를 현재 관측에서 다시 만들어 낼 수 있는지에 달려 있습니다. ReAct 루프의 본질은 궤적을 계속 추가해 모델이 작업을 앞으로 진전시키는 것입니다.
...
4. ★★ 제거 실험에서는 “도구 결과 피드백”이 없자 에이전트가 반복 예산을 다 쓸 때까지 재시도를 되풀이했습니다. 프로덕션 환경에서는 도구 결과 누락 외에 어떤 상황이 이런 루프를 일으킬 수 있을까요? 어떤 탐지 및 종료 메커니즘을 설계하겠습니까?
