...
يمكن التعبير عن أدنى تحقيق هندسي للوكيل الحديث بصيغة موجزة واحدة: **الوكيل = LLM (نموذج لغوي كبير، Large Language Model) + السياق + الأدوات**. وعلامات الجمع هنا تدل على تركيب مكوّنات هندسية، لا على تعريف صوري في التعلم المعزَّز؛ والأهم أن هذه الصيغة لا تصف إلا ما يقع داخل حدود الوكيل، و**لا تشمل الـ Environment (البيئة) التي يتفاعل معها الوكيل**. وكل لفظ فيها ينبغي فهمه فهمًا واسعًا لكن واضح الحدود:
...
- **تعريفات الأداة**: تعلن الأسماء والأوصاف الوظيفية وتنسيقات المعلمات للأدوات المتاحة للوكيل. بدون تعريفات الأدوات، لا يستطيع الوكيل التعرف على أي أدوات أو استدعائها - لكنه لا يصمت لهذا السبب؛ ستُظهر دراسة الاستئصال (التجربة 1-1) ما يفعله بدلًا من ذلك. تشكل تعريفات الأداة، جنبًا إلى جنب مع موجّه النظام، **البادئة الثابتة** التي تظل دون تغيير طوال المحادثة. (هذا هو النمط الأساسي؛ منذ عام 2026، يمكن لأطر الإنتاج أيضًا تحميل مخططات الأداة الكاملة عند الطلب في نهاية السياق دون كسر البادئة - راجع قسم تعريفات الأداة في الفصل 2 والفصل 4.)
...
وللتحقق من أن كل مكوّن لا غنى عنه حقًا، فأقصر الطرق هي **دراسة الحذف** (Ablation Study): تمامًا كما يستبعد الطبيب الأسباب واحدًا بعد آخر عند التشخيص — نحذف المكوّن A وننظر هل ما زال النظام يعمل على نحو سليم، ثم نحذف المكوّن B، وهكذا، لنحكم على إسهام كل مكوّن. وقد اختبرت التجربة 1-1 المكوّنات الخمسة السابقة اختبارًا منهجيًا وفق هذا المنطق بعينه.
...
>
> كشفت النتائج التجريبية عن دور كل مكون من مكونات السياق، وكشفت أيضًا أنها ليست متساوية الأهمية. **تعريفات الأداة** (جزء من البادئة الثابتة) هي أساس قدرة الوكيل على التصرف؛ وبدونها، لا يستطيع الوكيل استدعاء أي أداة. غير أن فقدان القدرة على التصرف لا يعني الصمت: إذ يظل النموذج يعيد إجابة منسّقة بعناية وبنبرة واثقة، لكن أرقامها تأتي من الذاكرة البارامترية لا من الملاحظات، وتُعرض تمامًا كما تُعرض إجابة مستمدة فعلًا من مخرجات الأدوات. أما أن يرفض صراحةً أو يختلق في الحال فيتوقف أساسًا على معدل الهلوسة ودرجة الصدق لدى النموذج نفسه؛ ووضع قيد في الموجّه من نوع «لا تقدّر أسعار الصرف بنفسك» يخفض احتمال الاختلاق ولا يلغيه. **نتائج الأداة** هي المفتاح للتحكم في الحلقة المغلقة؛ فبدونها ينفّذ الوكيل «على العمياء» ويعيد المحاولة حتى يستنفد ميزانية التكرارات. **عملية الاستدلال** (جزء الاستدلال في الرسائل المساعدة) تسجّل «لماذا اتُّخذت الخطوة»، بينما تسجّل نتائج الأداة «ما الذي حدث»؛ وحين يمكن إعادة بناء الأولى من الثانية، فإن إسقاطها من السجل لا يكلّف شيئًا يُذكر. **سجل الرسائل** (رسائل المستخدم، ورسائل المساعد، ونتائج الأداة من الجولات السابقة) يمنع العمليات المتكررة ويتجنب تكرار نفس الأخطاء.
>
> الرؤية الأساسية للتجربة: **يحدد السياق ما يستطيع الوكيل رؤيته، ولا يستطيع الوكيل اتخاذ القرار إلا بناءً على ما يراه**. غير أن المكونات ليست متكافئة؛ فالمعيار هو ما إذا كانت المعلومات التي يحملها المكوّن قابلة لإعادة البناء من مكان آخر - والادعاءات من نوع «كل مكوّن لا غنى عنه» ينبغي قياسها لا افتراضها، كما أن النماذج تتغير بسرعة تكفي لأن تصل التجربة نفسها على نموذج أحدث إلى نتيجة مختلفة. وثمة نقطة أخرى أهم في الممارسة الهندسية: **«أن يعطي إجابة» ليس «أن ينجز المهمة»**. فحين يغيب أحد مكونات السياق، لا يكون الإخفاق المعتاد خروجًا بخطأ، بل إجابة تبدو بلا عيب.
...
لنفهم **مسار** الوكيل (trajectory) من خلال مثال ملموس هو تجميع الإيرادات عبر عملات متعددة. المسار هو سجل الرسائل الذي يتراكم باطراد أثناء تنفيذ الوكيل للمهمة: رسائل المستخدم، وردود النموذج (بما فيها مسار التفكير واستدعاءات الأدوات)، ونتائج تنفيذ الأدوات. وفي كل استدعاء لـ LLM، يتألف السياق الكامل الذي يتلقاه من جزأين: **البادئة الثابتة** (مطالبة النظام + تعريفات الأدوات) و**المسار** (سجل الرسائل الديناميكي) (الشكل 1-4). ويكشف هذا حقيقة أساسية: **سياق الوكيل = البادئة الثابتة + المسار**. وعلى وجه التحديد، تقابل البادئةُ الثابتة أول بندين من المكوّنات الخمسة السابقة (مطالبة النظام + تعريفات الأدوات)، ويقابل المسارُ البنودَ الثلاثة الأخيرة (رسالة المستخدم + رد النموذج + نتيجة تنفيذ الأداة، وهي تنمو مع كل تفاعل). واستنادًا إلى هذا السياق الكامل يولّد LLM استجابة الخطوة التالية، ثم تُضاف هذه الاستجابة بدورها إلى المسار ليستعملها الاستدعاء التالي.

![الشكل 1-4: مسار الوكيل — حلقة ReAct لمهمة تجميع متعددة العملات](images/fig1-4.svg)

لننظر أولًا في الهيكل التشغيلي الأدنى. إنه يبيّن **كيف تعمل الآلية**: فالـ Model لا يتولى إلا تقرير الخطوة التالية، والـ Harness يتولى تركيب السياق والتحقق من الأدوات وتنفيذها، والـ Environment يتولى إحداث التغيّرات الحقيقية في الحالة وإنتاج الملاحظات. ويستمر هذا الكتاب لاحقًا في استعمال شبه الكود بأسلوب Python؛ وشبه الكود لا يُشغَّل مباشرة ولا يقابل SDK بعينه. أما الكود القابل للتنفيذ فعلًا فموجود في مستودع الكود المرافق للكتاب.
...
استنادًا إلى خبرة Anthropic، تتبع أنظمة الوكيل الناجحة ثلاثة مبادئ أساسية[^ch1-anthropic-building-effective-agents].
...
[^ch1-anthropic-building-effective-agents]: Anthropic. "Building effective agents", ديسمبر 2024. https://www.anthropic.com/engineering/building-effective-agents
...
أنماط التنسيق هي الطريقة التي ينظم بها Harness طبقة "السياق والأدوات" الخاصة به - فهي تحدد كيفية تدفق السياق بين استدعاءات LLM، وكيفية جدولة الأدوات، وما إذا كان مسار تنفيذ الوكيل قد تم إصلاحه مسبقًا أو تم إنشاؤه ديناميكيًا. لقد تطور تنسيق الوكيل من البسيط إلى المعقد، ولكل نمط حالات استخدام ومقايضات مناسبة. في تجربة Anthropic في العمل مع العشرات من الفرق التي تقوم ببناء وكلاء LLM[^ch1-anthropic-building-effective-agents]، نادرًا ما تستخدم التطبيقات الأكثر نجاحًا أطر عمل معقدة؛ يستخدمون أنماطًا بسيطة وقابلة للتركيب.

عند إنشاء تطبيق LLM، اتبع مبدأ الانتقال من البسيط إلى المعقد. ابدأ بالتفكير في استدعاء واحد لـ LLM. إذا أمكن حل المشكلة بتحسين الموجّهات والأمثلة داخل السياق، فلا تُدخل نظام وكلاء. وعندما تتطلب المهمة خطوات متعددة، فكّر في استخدام سير عمل للحالات التي يمكن تفكيكها بوضوح إلى مهام فرعية ثابتة. ولا تستخدم وكيلاً مستقلاً إلا عندما تحتاج إلى قرارات ديناميكية ومسارات تنفيذ مرنة. تذكّر أن أنظمة الوكلاء تستبدل عادةً زمن الاستجابة والكلفة بأداء أفضل في المهمة، لذلك زِن بعناية ما إذا كانت هذه المقايضة تستحق ذلك.

من الأمثلة المضادة الشائعة أن تبدأ ببناء سير عمل بالغ التعقيد أو نظام متعدد الوكلاء. فعند تصميم وكيل «لاستخلاص ذكريات شخصية من مليون رسالة محادثة»، ترسم بعض نماذج الذكاء الاصطناعي بسرعة خط إنتاج يبدو صارمًا: تقسيم المحادثات إلى مقاطع، ثم تكليف وكلاء على التوالي بالاستخراج، والتحقق من الأدلة، وحلّ الهويات، وتنظيم الذكريات، ومراجعة الدمج، وأخيرًا إنشاء رسم بياني للحقائق وسجل للتغطية وإصدارات غير قابلة للتغيير. يبدو كل مكوّن منطقيًا بمفرده، لكن جمعها ينتج نظامًا غير كفؤ إلى حد بعيد وغير موثوق. ولأن مخطط التنفيذ في سير العمل المعقد ثابت، يدفع كل استثناء جديد بسهولة إلى إضافة عقدة أخرى؛ فتزداد البنية تعقيدًا بينما تتراجع قابليتها للتعميم. وهكذا تُثبَّت في سير العمل أحكام دلالية كان يمكن للنموذج اتخاذها بالاعتماد على السياق.

لذلك، **أهمية Harness لا تعني أن زيادة تعقيده تجعله أفضل**. ويلخّص موقع Manus هذه المقايضة بعبارة “Less structure, more intelligence.”[^ch1-manus-less-structure] ابدأ بمنح وكيل ذي قدرة كافية هدفًا واضحًا، والسياق الضروري، وأدوات قابلة للتركيب، ووضّح له باللغة الطبيعية كيف يتحقق ويقارن ويعالج التعارضات كما يفعل الإنسان. أما البرنامج فلا ينبغي أن يثبّت إلا الحدود التي يجب أن تظل صحيحة دائمًا، مثل الصلاحيات، ومنع الكتابة فوق المواد الأصلية، والنشر الذرّي. ولا تحوّل خطوة إلى أداة تحقق متخصصة أو وكيل مستقل أو عملية حتمية إلا إذا فرضت قيود العمل ذلك بذاتها، أو كشفت التقييمات مرارًا عن نمط إخفاق ثابت. البنية الجيدة لا تستبق كل تفكير الوكيل، بل تحرس الحدود وتعيد إلى النموذج مساحة القرار داخلها.
...
وثمة طريقة مزج أخرى: **أن يكتب الوكيل المستقل سير العمل أولًا، ثم يتولى سير العمل التنفيذ**. فبعد قراءة المهمة يقرر الوكيل بنفسه البنية ويولّد شيفرة تنسيق؛ وما إن تُولَّد الشيفرة حتى تعود مرحلة التنفيذ إلى حتمية سير العمل. وبهذا نحتفظ بمرونة الوكيل المستقل أمام المهام غير المعروفة، من دون إلزام النموذج باتخاذ قرار عند كل جدولة. ويناقش الفصل العاشر هذا النمط بالتفصيل.
...
**السياق هو العامل الحاسم**: يتكون السياق من بادئة ثابتة (موجّه النظام + تعريفات الأداة) ومسار ديناميكي (سجل الرسائل). يوضح الاستئصال أن المكونات ليست متكافئة: فإزالة تعريفات الأداة أو نتائجها تسلب مباشرةً القدرة على التصرف أو على إغلاق الحلقة، أما كلفة إزالة المكوّنين الآخرين فتتوقف على ما إذا كانت تلك المعلومات قابلة لإعادة البناء من الملاحظات الحالية. جوهر حلقة ReAct هو إلحاق المسار مرارًا وتكرارًا، لذلك يستمر النموذج في تقدم المهمة.
...
4. ★★ في تجربة الاستئصال، يدفع غياب "التغذية الراجعة لنتائج الأداة" الوكيلَ إلى إعادة المحاولة حتى يستنفد ميزانية التكرارات. في بيئة الإنتاج، إلى جانب فقدان نتائج الأداة، ما هي المواقف الأخرى التي قد تدفع الوكيل إلى حلقة من هذا النوع؟ ما هي آليات الكشف والإنهاء التي ستصممها؟
