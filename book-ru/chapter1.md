...
Минимальную инженерную реализацию современного агента можно выразить одной короткой формулой: **Agent = LLM (большая языковая модель, Large Language Model) + контекст + инструменты**. Плюсы здесь обозначают сочетание инженерных компонентов, а не формальное определение из обучения с подкреплением; и, что важнее, формула описывает только то, что находится внутри границы агента, и **не включает Environment (среду), с которой агент взаимодействует**. Каждое слово в ней следует понимать широко, но с чёткой границей:
...
- **Определения инструментов** (Tool Definitions): объявляют имена доступных агенту инструментов, описания их функций и формат параметров. Без определений инструментов агент не сможет распознать и вызвать ни один инструмент — но и не остановится из-за этого; абляционное исследование (эксперимент 1-1) покажет, что он делает вместо этого. Определения инструментов вместе с системным промптом образуют **статический префикс**, неизменный на протяжении диалога (это базовый режим; с 2026 года в производственных фреймворках полная schema инструментов также может по мере необходимости динамически подгружаться в конец контекста, не нарушая префикс, подробнее — в разделе об определениях инструментов второй главы и в четвёртой главе).
...
>
> Результаты эксперимента раскрывают роль каждого компонента контекста — и то, что они не одинаково важны. **Определения инструментов** (Tool Definitions, часть статического префикса) — основа способности агента действовать; без них агент не может вызвать ни один инструмент. Но утрата способности действовать не означает молчания: модель по-прежнему выдаёт аккуратно оформленный и уверенно сформулированный ответ, только числа в нём взяты из параметрической памяти, а не из наблюдений, и свёрстаны ровно так же, как ответ, действительно полученный из результатов инструментов. Откажется ли модель честно или сочинит на месте — зависит главным образом от её собственного уровня галлюцинаций и честности; ограничение в промпте вроде «не оценивайте курсы самостоятельно» лишь снижает вероятность выдумки, но не устраняет её. **Результаты выполнения инструментов** (Tool Results) — ключ к замкнутому контуру управления; без них агент действует «вслепую» и повторяет попытки, пока не исчерпает бюджет итераций. **Процесс размышления** (часть reasoning в ответах модели) фиксирует «почему был сделан шаг», а результаты инструментов — «что произошло»; когда первое восстанавливается из второго, отказ от него в истории почти ничего не стоит. **История сообщений** (сообщения пользователя, ответы модели и результаты выполнения инструментов из предыдущих раундов) предотвращает избыточные операции и помогает не повторять одни и те же ошибки.
>
> Ключевой вывод этого эксперимента таков: **контекст определяет то, что агент способен видеть, а агент может принимать решения только на основе того, что он видит**. Но компоненты не равноценны: мерилом служит то, можно ли восстановить несомую компонентом информацию откуда-то ещё. Утверждения вроде «ни один компонент нельзя убрать» нужно измерять, а не предполагать, и модели сменяются достаточно быстро, чтобы та же абляция на более новой модели вполне могла дать иной результат. Ещё один момент важнее для инженерной практики: **«выдал ответ» — не то же самое, что «выполнил задачу»**. При нехватке компонента контекста типичный отказ — не выход с ошибкой, а ответ, который выглядит безупречно.
...
Начнём с минимального рабочего скелета. Он показывает, **как устроен сам механизм**: Model только решает, каким будет следующий шаг, Harness собирает контекст, проверяет и выполняет инструменты, а Environment порождает реальные изменения состояния и наблюдения. Дальше в книге тоже используется псевдокод в стиле Python; он не запускается напрямую и не соответствует какому-либо SDK. Конкретный исполняемый код лежит в сопутствующем репозитории книги.
...
По опыту Anthropic, успешные агентные системы следуют трём основным принципам[^ch1-anthropic-building-effective-agents].
...
[^ch1-anthropic-building-effective-agents]: Anthropic. "Building effective agents", декабрь 2024 г.. https://www.anthropic.com/engineering/building-effective-agents
...
Паттерн оркестрации — это способ организации на уровне «контекста и инструментов» внутри Harness: он определяет, как контекст перетекает между вызовами LLM, как планируются вызовы инструментов и задаётся ли путь исполнения агента заранее или формируется динамически. Способы оркестрации агентных систем прошли путь развития от простого к сложному, и у каждого паттерна есть свои подходящие сценарии и компромиссы, которые приходится учитывать. По опыту Anthropic, накопленному в сотрудничестве с десятками команд при построении LLM-агентов[^ch1-anthropic-building-effective-agents], самые успешные реализации чаще всего используют не сложные фреймворки, а простые, компонуемые паттерны.

При построении LLM-приложений стоит придерживаться принципа «от простого к сложному». Сначала подумайте об одиночном вызове LLM. Если задачу можно решить оптимизацией промпта и примеров в контексте, не вводите агентную систему. Когда требуется многошаговая обработка, рассмотрите рабочий процесс для сценариев, которые чётко раскладываются на фиксированные подзадачи. И только когда нужны динамические решения и гибкие пути исполнения, применяйте автономного агента. Помните: агентные системы обычно платят задержкой и стоимостью за более высокую производительность в задачах, поэтому тщательно взвешивайте, оправдан ли такой обмен.

Распространённый антипример — с самого начала строить чрезвычайно сложный рабочий процесс или мультиагентную систему. Например, если попросить спроектировать агента, который «извлекает личные воспоминания из миллиона сообщений чата», некоторые модели ИИ быстро рисуют внешне строгий конвейер: сначала разбить диалоги на фрагменты, затем последовательно назначить агентов для извлечения, проверки доказательств, разрешения идентичностей, упорядочивания памяти и проверки слияния, а в конце построить граф фактов, журнал покрытия и неизменяемые версии. Каждый компонент по отдельности выглядит разумно, но вместе они образуют крайне неэффективную и ненадёжную систему. Топология исполнения сложного рабочего процесса фиксирована, поэтому каждое новое исключение легко приводит к добавлению очередного узла: архитектура становится всё сложнее, а универсальность — всё ниже. Семантические решения, которые модель могла бы принимать по контексту, оказываются жёстко прописаны в рабочем процессе.

Поэтому **важность Harness не означает, что чем он сложнее, тем лучше**. Сайт Manus формулирует этот компромисс как “Less structure, more intelligence.”[^ch1-manus-less-structure] Сначала дайте достаточно способному агенту ясную цель, необходимый контекст и компонуемые инструменты, а естественным языком объясните, как проверять, сравнивать и разрешать конфликты подобно человеку. В программе следует жёстко закрепить только границы, которые должны соблюдаться всегда: разрешения, запрет на перезапись исходных материалов, атомарную публикацию и подобные требования. Выделять соответствующий шаг в специализированный верификатор, независимого агента или детерминированный процесс стоит лишь тогда, когда этого требуют сами бизнес-ограничения или оценки многократно выявляют устойчивый тип сбоя. Хорошая структура не разыгрывает заранее все рассуждения агента; она охраняет границы и возвращает модели пространство решений внутри них.
...
Есть и ещё один способ сочетания: **сначала автономный агент пишет рабочий процесс, а затем этот рабочий процесс его исполняет**. Прочитав задачу, агент сам решает, какой будет топология, и генерирует код оркестрации; как только код готов, стадия исполнения возвращается к детерминированности рабочего процесса. Так сохраняется гибкость автономного агента перед незнакомой задачей, но модели не приходится принимать решение на каждом шаге планирования. Подробно эта форма разбирается в главе 10.
...
**Глаза (контекст) — решающий фактор**: контекст состоит из статического префикса (системный промпт + определения инструментов) и динамической траектории (история сообщений). Абляционное исследование показывает, что компоненты не равноценны: удаление определений инструментов или результатов их выполнения прямо лишает агента способности действовать или замыкать контур, тогда как цена удаления двух остальных зависит от того, восстановима ли эта информация из текущих наблюдений. Суть цикла ReAct — заставлять модель непрерывно продвигать задачу, постоянно дописывая траекторию.
...
4. ★★ В абляционном исследовании отсутствие «обратной связи о результатах инструментов» заставляет агента повторять попытки, пока не исчерпается бюджет итераций. Какие ещё ситуации в продакшене, помимо отсутствия результатов инструментов, могут вогнать агента в такой цикл? Какой механизм обнаружения и остановки вы бы спроектировали?
