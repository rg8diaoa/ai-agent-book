...
現代の Agent の最小の工学的実装は、簡潔な一つの式で表せます。**Agent = LLM（大規模言語モデル、Large Language Model）+ コンテキスト + ツール**。ここでのプラス記号は工学的コンポーネントの組み合わせを表すもので、強化学習における形式的定義ではありません。さらに重要なのは、この式が Agent の境界の内側の実装だけを述べていて、**Agent が相互作用する Environment（環境）を含まない**ことです。式の中の各語は、広い意味で、しかし境界を明確にして理解する必要があります。
...
前で、モデルが強化学習を通じてツール呼び出しの方策をネイティブな能力として内在化できることを論じました。しかし Agent の振る舞いの変化は、訓練の段階だけで起こるわけではありません。更新が起こる場所と持続時間に沿って、三つの補い合う経路として理解できます（図1-2）。すなわち、タスク内でのコンテキスト適応、タスクをまたぐ外部成果物（artifact）の更新、そして訓練サイクルにおけるパラメータの更新です。

![図1-2 Agent の 3 つの学習パラダイム](images/fig1-2.svg)

**コンテキスト適応**は現在のタスクの中で起こります。事例、状態、検索結果がコンテキストに入れば、モデルは直ちに振る舞いを調整できますが、それによって次のセッションの永続的な状態が変わるわけではありません。その利点は速く低コストであること、限界はコンテキストウィンドウと情報の組み立て方に縛られることです。第 2 章では、この適応がどう働くのかを詳しく論じます。

変化をタスクをまたいで残すには、**外部成果物**を更新できます。事実と経験を知識ドキュメントに整理し、言語化できる方策を Prompt や Skill に書き込み、決定論的な手順と制約をプログラムや Harness として書き下すのです。これらの成果物は監査でき、改訂でき、実行時にはやはりコンテキストかツールのインタフェースを通じて Agent に使われます。第 3 章から第 5 章までが知識とプログラムの基礎を与え、第 9 章では、評価済みの実行軌跡からこうした更新をどう生み出すかを論じます。

目標が医用画像の理解、自然な言語のスタイル、暗黙的な意思決定方策といった高次元の能力であるとき、外部のルールでは完全に表現しきれないため、ポストトレーニングによって**モデルのパラメータ**を更新する必要があります。パラメータの更新は導入コストが高いものの、自然で広い汎化能力を形づくります。第 8 章でその手法を体系的に紹介します。したがってこの三つの経路は互いに排他的な分類ではなく、異なる時間スケールで協調する仕組みです。コンテキストはその場の適応を、外部成果物は制御可能な蓄積を、パラメータは明示的に表しにくい能力の内在化を担います。
...
- **ツール定義**（Tool Definitions）：Agent が使えるツールの名前、機能の説明、パラメータの形式を宣言します。ツール定義がなければ、Agent はいかなるツールも識別・呼び出しできませんが、それで黙り込むわけでもありません。アブレーション実験（実験 1-1）でこの点を見ていきます。ツール定義はシステムプロンプトとともに、対話の中で変わらない**静的プレフィックス**を構成します（これは基本的なパターンです。2026 年以降、本番フレームワークではツールの完全な schema をオンデマンドでコンテキストの末尾に動的に読み込み、プレフィックスを壊さないこともできます。第 2 章のツール定義の節および第 4 章を参照）。
...
各コンポーネントがいずれも欠かせないものかどうかを検証する最も直接的な方法は、**アブレーション実験**（Ablation Study）です。医師が診断のときに病因を一つずつ排除していくように、まず A のコンポーネントを取り除いてシステムがなお正常かを見て、次に B のコンポーネントを取り除き、以下同様にして、各コンポーネントの寄与を判断します。実験 1-1 はまさにこの考え方に沿って、上記の 5 つのコンポーネントに対して体系的なテストを行いました。
...
>
> 実験結果は、各コンテキストコンポーネントの役割と、それらが同等に重要ではないことを明らかにしました。**ツール定義**（Tool Definitions、静的プレフィックスの一部）は Agent の行動能力の基礎であり、それがなければ Agent はいかなるツールも呼び出せません。ただし、行動能力を失うことは沈黙を意味しません。モデルは体裁の整った、断定的な口調の答えをそのまま返してきます。ただしその数値は観測ではなくパラメトリックメモリ（学習で獲得した記憶）から来ており、本当に観測にもとづいて導かれた答えとまったく同じ体裁で並びます。素直に断るのか、その場ででっち上げるのかは、主にモデル自身のハルシネーション率と誠実さ（honesty）で決まります。プロンプトに「自分でレートを推定してはならない」といった制約を書いても、でっち上げの確率を下げるだけで、なくすことはできません。**ツール実行結果**（Tool Results）は閉ループ制御の鍵であり、それが欠けると Agent は「盲目的に」実行し、イテレーション予算を使い切るまで再試行を繰り返します。**思考過程**（モデル返答の中の reasoning の部分）が記録するのは「なぜそうしたのか」であり、ツール実行結果が記録するのは「何が起きたのか」です。前者を後者から再構成できる場合、それを履歴から落としてもコストはほとんどありません。**履歴メッセージ**（それ以前のラウンドのユーザーメッセージ、モデル返答、ツール実行結果）は冗長な操作を防ぎ、同じ誤りを繰り返すことを避けます。
>
> この実験の核となる洞察はこうです。**コンテキストは Agent が何を見られるかを決め、Agent は自分が見た情報にもとづいてしか意思決定できない**。ただしコンポーネントは等価ではありません。基準は、それが担う情報を他から再構成できるかどうかです。「どのコンポーネントも欠かせない」といった主張は実測にもとづくべきであり、しかもモデルの世代交代は速いので、同じアブレーションを新しいモデルで回せば違う結論になることも十分あります。もう一つ、エンジニアリングの実務ではさらに重要な点があります。**「回答を返した」ことは「タスクを完了した」ことではありません**。コンテキストが欠けたときの典型的な失敗は、エラー終了ではなく、一見まったく隙のない答えなのです。
...
まず最小の実行スケルトンを見ましょう。これが示すのは**機構がどう動くか**です。Model は次の一手を決めるだけ、Harness はコンテキストの組み立てとツールの検証・実行を担い、Environment は実際の状態変化と観測を生み出します。本書ではこの先も Python 風の疑似コードを用います。疑似コードはそのままでは実行できず、特定の SDK に対応するものでもありません。具体的な実行可能コードは本書の付属コードリポジトリにあります。
...
Anthropic の経験によれば、成功する Agent システムは 3 つの核となる原則に従っています[^ch1-anthropic-building-effective-agents]。
...
オーケストレーションパターンは、Harness における「コンテキストとツール」のレベルでの組織のしかたです。コンテキストが LLM 呼び出しの間をどのように流れるか、ツールがどのようにスケジューリングされるか、そして Agent の実行経路があらかじめ設定されているのか動的に生成されるのかを決めます。Agent システムのオーケストレーションのしかたは、単純なものから複雑なものへと進化してきました。各パターンにはそれぞれ適した場面と、天秤にかけるべきトレードオフがあります。Anthropic が数十のチームと協力して LLM Agent を構築した経験によれば[^ch1-anthropic-building-effective-agents]、最も成功する実装は、往々にして複雑なフレームワークを使うことではなく、シンプルで組み合わせ可能なパターンを採ることにあります。

LLM アプリケーションを構築するときは、「単純なものから複雑なものへ」という原則に従うべきです。まず単一の LLM 呼び出しを検討します。プロンプトとコンテキストの例を最適化するだけで問題を解決できるなら、Agent システムを持ち込まないことです。複数ステップの処理が必要になったら、固定的なサブタスクへ明確に分解できる場面では、ワークフローの使用を検討します。動的な意思決定と柔軟な実行経路が必要なときにだけ、自律 Agent を使います。Agent システムは通常、レイテンシとコストと引き換えにより良いタスク性能を得るものであり、その引き換えが見合うかどうかを慎重に判断する必要があります。

よくある反面教師は、最初から非常に複雑なワークフローやマルチ Agent システムを構築することです。たとえば、「100 万件のチャットメッセージから個人の記憶を抽出する」Agent を設計させると、一部の AI モデルは、すぐに一見厳密なパイプラインを描きます。まず会話を断片に分割し、抽出、証拠の検証、人物同定、記憶の整理、マージのレビューを担う Agent を順番に配置し、最後に事実グラフ、カバレッジ台帳、不変バージョンを構築するというものです。各部品は単独で見ればもっともらしいのですが、組み合わせると非常に非効率で信頼性の低いシステムになります。複雑なワークフローは実行トポロジーが固定されているため、新しい例外に出会うたびにノードを追加しがちです。アーキテクチャは複雑になる一方で、汎用性は下がっていきます。本来ならモデルがコンテキストに基づいて処理できた意味上の判断まで、ワークフローに固定されてしまいます。

したがって、**Harness が重要だからといって、複雑なほどよいわけではありません**。Manus の公式サイトは、このトレードオフを “Less structure, more intelligence.” と表現しています[^ch1-manus-less-structure]。まず、十分な能力を持つ Agent に、明確な目標、必要なコンテキスト、組み合わせ可能なツールを与え、人間と同じように検証、比較、矛盾の処理を行う方法を自然言語で伝えます。プログラムに固定するのは、権限、原資料を上書きしないこと、アトミックな公開など、常に成立しなければならない境界だけです。業務上の制約そのものが要求する場合、または評価によって同じ種類の失敗が繰り返し確認された場合に限って、対応するステップを専用の検証器、独立した Agent、決定的なプロセスへ昇格させます。よい構造とは、Agent の思考をすべて先回りして決めるものではありません。境界を守り、その内側の意思決定空間をモデルに返すものです。
...
混合のしかたはもう一つあります。**まず自律 Agent にワークフローを書かせ、そのワークフローに実行させる**という形です。Agent はタスクを読んだうえで自らトポロジーを決め、オーケストレーションのコードを生成します。コードができてしまえば、実行段階はワークフローの決定性に戻ります。こうすれば、未知のタスクに向き合う自律 Agent の柔軟性を保ちながら、一回ごとのスケジューリングにモデルを関与させずに済みます。この形態は第 10 章で詳しく述べます。
...
**目（コンテキスト）が決定的な要因である**：コンテキストは静的プレフィックス（システムプロンプト + ツール定義）と動的な軌跡（メッセージ履歴）から構成されます。アブレーション実験が示したのは、各コンポーネントが等価ではないということです。ツール定義やツール実行結果を取り除くと行動能力や閉ループ能力そのものが失われますが、残る二つを取り除いたときのコストは、その情報を現在の観測から再構成できるかどうかで決まります。ReAct ループの本質は、軌跡を絶えず追加していくことで、モデルにタスクを継続的に前へ進めさせることにあります。
...
4. ★★ アブレーション実験では、「ツール結果のフィードバック」の欠如により、Agent はイテレーション予算を使い切るまで再試行を繰り返しました。本番環境では、ツール結果の欠如のほかに、どのような状況が Agent をこの種のループに陥らせうるでしょうか。あなたならどのような検知と終了の仕組みを設計しますか。
