...
La realización mínima de ingeniería de un Agente moderno cabe en una fórmula concisa: **Agente = LLM (modelo de lenguaje grande, Large Language Model) + contexto + herramientas**. Los signos de suma denotan aquí una combinación de componentes de ingeniería, no una definición formal del aprendizaje por refuerzo; y, más importante todavía, la fórmula describe solo lo que queda dentro de la frontera del Agente y **no incluye el Environment (entorno) con el que el Agente interactúa**. Cada término debe leerse en sentido amplio, pero con una frontera clara:
...
La taxonomía completa y los principios de diseño de estos cinco tipos de herramientas se desarrollan en el capítulo 4. La calidad del diseño de las herramientas determina directamente hasta dónde puede llegar un Agente: si la definición de la interfaz no es clara, el modelo usará las herramientas de cualquier manera; si el tratamiento de errores es deficiente, basta con que una llamada falle para que el Agente quede bloqueado; y si el control de permisos es demasiado laxo, en cuanto el Agente se equivoque las consecuencias serán difíciles de revertir. La difusión del estándar MCP (Model Context Protocol, protocolo de contexto de modelo) está facilitando la integración de herramientas.
...
El contexto es toda la información que un Agente puede ver en cada punto de decisión. Igual que una persona necesita tener a la vista todo el material desplegado sobre la mesa para decidir —el enunciado de la tarea, los manuales de referencia, el registro de comunicaciones previas, los datos más recientes—, la ventana de contexto del Agente es su «campo visual». Desde la perspectiva de la API (detallada en el capítulo 2), el contexto de cada llamada al LLM consta de las cinco partes siguientes:

- **Prompt del sistema** (System Prompt): a diferencia del prompt que el usuario escribe en cada turno, el prompt del sistema lo redacta el desarrollador y permanece invariable durante toda la conversación; equivale a la «descripción del puesto» del Agente, pues define su identidad, sus permisos y sus reglas de conducta. Diseñándolo con cuidado mediante ingeniería de prompts (Prompt Engineering) damos forma al modo de trabajar del Agente. El prompt del sistema también incorpora la **memoria del usuario** que persiste entre sesiones (preferencias del usuario, comportamiento pasado, datos de contexto personal y demás información personalizada; véase el capítulo 3) y el estado del entorno inyectado dinámicamente.
- **Definiciones de herramientas** (Tool Definitions): declaran el nombre, la descripción funcional y el formato de parámetros de las herramientas de que dispone el Agente. Sin definiciones de herramientas el Agente no puede reconocer ni invocar ninguna, pero eso no hace que se detenga: el estudio de ablación (experimento 1-1) mostrará qué ocurre entonces. Las definiciones de herramientas, junto con el prompt del sistema, forman el **prefijo estático** que se mantiene invariable a lo largo de la conversación (este es el modo básico; desde 2026, en los marcos de producción el esquema completo de una herramienta también puede cargarse dinámicamente al final del contexto sin romper el prefijo, como se detalla en la sección sobre definiciones de herramientas del capítulo 2 y en el capítulo 4).
- **Mensajes del usuario** (User Messages): la entrada procedente del usuario. Los mensajes del usuario también pueden incorporar **conocimiento externo** recuperado dinámicamente mediante RAG (generación aumentada por recuperación, Retrieval-Augmented Generation; véase el capítulo 3), que cubre información posterior al corte de los datos de entrenamiento o conocimiento privado de un dominio.
- **Respuestas del modelo** (Assistant Messages): las respuestas que el modelo generó anteriormente, con hasta tres partes: el proceso de razonamiento (`reasoning`, la cadena de pensamiento interna, que mantiene la coherencia del razonamiento y la explicabilidad de las decisiones), el contenido textual (`content`, la respuesta dirigida al usuario) y las peticiones de llamada a herramientas (`tool_calls`, la forma en que el Agente actúa). En una respuesta concreta no tienen por qué aparecer las tres a la vez: cuando el Agente decide llamar a una herramienta suele haber solo `reasoning` + `tool_calls`, y cuando da la respuesta final suele haber solo `reasoning` + `content`.
- **Resultados de ejecución de herramientas** (Tool Results): lo que devuelve el marco del Agente tras ejecutar la herramienta. Estos resultados son la base directa del siguiente razonamiento del Agente y le permiten además aprender de lo ejecutado y evitar repetir errores.

Los dos primeros elementos (prompt del sistema + definiciones de herramientas) forman el prefijo estático; los tres últimos (mensaje del usuario + respuesta del modelo + resultado de ejecución de la herramienta) forman el historial dinámico de mensajes, que crece con cada interacción. Estas cinco partes constituyen conjuntamente el contexto de cada inferencia del LLM.

Para comprobar si cada componente es realmente imprescindible, el método más directo es un **estudio de ablación** (Ablation Study): igual que un médico va descartando causas una a una en el diagnóstico —quitar el componente A y ver si el sistema sigue funcionando, luego quitar el B, y así sucesivamente— para juzgar la contribución de cada uno. El experimento 1-1 aplica exactamente este método a los cinco componentes anteriores.
...
>
> Los resultados revelaron el papel de cada componente y que no todos pesan igual. Las **Definiciones de Herramientas** son la base de la capacidad de acción; sin ellas, el Agente no puede llamar a ninguna herramienta. Pero perder la capacidad de actuar no equivale a callar: el modelo sigue devolviendo una respuesta bien formateada y de tono seguro, solo que sus cifras proceden de la memoria paramétrica y no de las observaciones, y se presentan igual que una respuesta realmente derivada de las herramientas. Que rechace con franqueza o invente sobre la marcha depende sobre todo de la tasa de alucinación y la honestidad del propio modelo; una restricción en el prompt como «no estimes tú los tipos de cambio» reduce la probabilidad de invención, pero no la elimina. Los **Resultados de Herramientas** son clave para el control de bucle cerrado; sin ellos el Agente actúa «a ciegas» y reintenta hasta agotar su presupuesto de iteraciones. El **proceso de razonamiento** registra *por qué* se dio un paso, mientras que los resultados registran *qué ocurrió*; cuando lo primero puede reconstruirse a partir de lo segundo, descartarlo del historial casi no cuesta nada. El **historial de mensajes** evita operaciones redundantes y no repetir los mismos errores.
>
> La conclusión central: **el contexto determina qué información tiene el Agente al decidir, y el Agente solo puede decidir basándose en esa información**. Pero los componentes no son equivalentes: la medida es si la información que aporta cada uno puede reconstruirse desde otro sitio, algo que hay que medir y no suponer, y los modelos cambian lo bastante rápido como para que la misma ablación en uno más nuevo llegue a otra conclusión. Y algo más importante en la práctica: **«dar una respuesta» no es «completar la tarea»**; cuando falta un componente, el fallo típico no es un error sino una respuesta que parece impecable.
...
El patrón central mediante el cual un Agente ejecuta una tarea se llama **ReAct** (Reasoning + Acting). Aunque el nombre solo recoge dos palabras, razonar (Reasoning) y actuar (Acting), el bucle real consta de tres etapas: el modelo primero **razona** sobre qué conviene hacer, después llama a una herramienta para **actuar**, y luego **observa** el resultado devuelto por la herramienta y sigue razonando el paso siguiente. Este bucle «pensar → hacer → mirar → pensar → hacer → mirar» se repite sin cesar hasta que la tarea se completa.

Entendamos la **trayectoria** (trajectory) del Agente mediante un ejemplo concreto: la agregación de ingresos en varias monedas. La trayectoria es el historial de mensajes que el Agente va acumulando mientras ejecuta la tarea —mensajes del usuario, respuestas del modelo (incluidos el proceso de razonamiento y las llamadas a herramientas) y resultados de ejecución de las herramientas—. En cada llamada al LLM, el contexto completo que este recibe se compone de dos partes: el **prefijo estático** (prompt del sistema + definiciones de herramientas) y la **trayectoria** (el historial dinámico de mensajes) (figura 1-4). De aquí se desprende un hecho clave: **contexto del Agente = prefijo estático + trayectoria**. Concretamente, el prefijo estático corresponde a los dos primeros de los cinco componentes anteriores (prompt del sistema + definiciones de herramientas), y la trayectoria corresponde a los tres últimos (mensaje del usuario + respuesta del modelo + resultado de ejecución de la herramienta, que crecen con cada interacción). A partir de este contexto completo el LLM genera la respuesta del paso siguiente, que a su vez se anexa a la trayectoria para que la use la llamada posterior.

![Figura 1-4: Trayectoria del Agente, Bucle ReAct para una tarea de agregación multimoneda](images/fig1-4.svg)

Veamos primero el esqueleto mínimo de ejecución. Lo que muestra es **cómo funciona el mecanismo**: el Model solo decide el siguiente paso, el Harness ensambla el contexto y valida y ejecuta las herramientas, y el Environment produce los cambios de estado reales y las observaciones. En el resto del libro se sigue usando pseudocódigo de estilo Python; el pseudocódigo no es ejecutable y no corresponde a ningún SDK concreto. El código ejecutable está en el repositorio que acompaña al libro.
...
En este diseño, el más básico de todos, el contexto se va anexando continuamente: cada llamada al LLM ve la trayectoria completa, de modo que el modelo sabe en qué punto está la tarea, qué se ha intentado antes y qué resultados se obtuvieron. La estructuración de la trayectoria también hace el sistema fácil de explicar y depurar, pues el mensaje del usuario, la respuesta del modelo (proceso de razonamiento + llamadas a herramientas) y el resultado de ejecución quedan claramente separados. Yendo más lejos, analizar un gran número de trayectorias permite descubrir patrones de comportamiento del Agente, optimizar las rutas de decisión y mejorar el diseño de las herramientas; las trayectorias pueden además sedimentarse en una base de conocimiento, o servir para entrenar mejores modelos mediante aprendizaje por refuerzo, cerrando así el ciclo del aprendizaje a partir de la experiencia.
...
A estas alturas ya has entendido el principio de funcionamiento central de un Agente: el LLM, mediante el bucle ReAct y con la ayuda del contexto, usa herramientas para completar tareas. Los experimentos anteriores demuestran que este mecanismo básico funciona, pero también dejan al descubierto puntos frágiles evidentes: el modelo puede alucinar (inventar herramientas o parámetros que no existen), elegir la herramienta equivocada o ser incapaz de recuperarse por sí mismo ante un error. Entre una demo que funciona y un producto fiable media un abismo, y son precisamente esos puntos frágiles los que la ingeniería del Harness debe resolver. La primera mitad de este capítulo responde a qué es un Agente; la segunda, a cómo hacerlo funcionar de forma fiable en producción.
...
Según la experiencia de Anthropic, los sistemas de Agentes de éxito siguen tres principios fundamentales[^ch1-anthropic-building-effective-agents].
...
[^ch1-anthropic-building-effective-agents]: Anthropic. "Building effective agents", diciembre de 2024. https://www.anthropic.com/engineering/building-effective-agents
...
Los patrones de orquestación son la forma de organizar las capas de «contexto y herramientas» dentro del Harness—determinan cómo fluye el contexto entre las llamadas al LLM, cómo se programan las herramientas y si la ruta de ejecución del Agente está predefinida o se genera dinámicamente. La orquestación de los sistemas de Agentes ha evolucionado desde soluciones sencillas hasta otras más complejas; cada patrón tiene escenarios de aplicación adecuados y compromisos que deben sopesarse. Según la experiencia de Anthropic colaborando con decenas de equipos en la construcción de Agentes basados en LLM[^ch1-anthropic-building-effective-agents], las implementaciones más exitosas no suelen utilizar frameworks complejos, sino patrones sencillos y componibles.

Al construir aplicaciones basadas en LLM, debe seguirse el principio de «ir de lo sencillo a lo complejo». Primero, considere una única llamada al LLM. Si el problema puede resolverse optimizando el prompt y los ejemplos de contexto, no introduzca un sistema de Agentes. Cuando sea necesario un procesamiento de varios pasos, considere utilizar un workflow en escenarios que puedan descomponerse claramente en subtareas fijas. Utilice un Agente autónomo solo cuando sean necesarias decisiones dinámicas y rutas de ejecución flexibles. Recuerde que los sistemas de Agentes suelen intercambiar latencia y coste por un mejor rendimiento en las tareas, por lo que debe sopesarse cuidadosamente si este intercambio merece la pena.

Un contraejemplo habitual consiste en empezar construyendo un workflow muy complejo o un sistema multi‑Agente. Por ejemplo, al diseñar un Agente para «extraer recuerdos personales de un millón de mensajes de chat», algunos modelos de IA trazan enseguida una canalización aparentemente rigurosa: primero segmentan las conversaciones; después asignan, en secuencia, Agentes de extracción, verificación de evidencias, resolución de identidades, organización de recuerdos y revisión de la fusión; por último, crean un grafo de hechos, un registro de cobertura y versiones inmutables. Cada componente parece razonable por separado, pero el conjunto resulta muy ineficiente y poco fiable. Como la topología de ejecución de un workflow complejo es fija, cada nueva excepción tiende a añadir otro nodo: la arquitectura se vuelve cada vez más compleja y, al mismo tiempo, menos general. Las decisiones semánticas que el modelo podría tomar a partir del contexto terminan fijadas en el workflow.

Por tanto, **que el Harness sea importante no significa que un Harness más complejo sea mejor**. El sitio web de Manus resume esta compensación con «Less structure, more intelligence.»[^ch1-manus-less-structure] Primero hay que proporcionar a un Agente suficientemente capaz un objetivo claro, el contexto necesario y herramientas componibles, y explicarle en lenguaje natural cómo verificar, comparar y resolver conflictos como lo haría una persona. El programa solo debe fijar límites que siempre tengan que cumplirse, como los permisos, la prohibición de sobrescribir el material original y la publicación atómica. Únicamente cuando las propias restricciones del negocio lo exijan, o las evaluaciones revelen repetidamente un modo de fallo estable, debe convertirse el paso correspondiente en un verificador especializado, un Agente independiente o un proceso determinista. Una buena estructura no anticipa todo el razonamiento del Agente: protege los límites y devuelve al modelo el espacio de decisión que queda dentro de ellos.
...
En la práctica, los workflows y los Agentes autónomos no son una disyuntiva: muchos sistemas combinan ambos modos —los procesos críticos, con requisitos estrictos de cumplimiento, se resuelven con workflows para garantizar la fiabilidad, y las partes que exigen decisiones flexibles se delegan al modo autónomo—. Por ejemplo, n8n es un marco de código abierto maduro para la automatización de workflows en el que el desarrollador construye el Agente arrastrando componentes en una interfaz visual, y donde pueden convivir en un mismo sistema nodos de workflow y nodos de Agente autónomo.
...
**Los ojos (el contexto) son el factor decisivo**: el contexto consta de un prefijo estático (prompt del sistema + definiciones de herramientas) y una trayectoria dinámica (historial de mensajes). Los estudios de ablación demuestran que los componentes no son equivalentes: quitar las definiciones o los resultados de herramientas arrebata directamente la capacidad de actuar o de cerrar el bucle, mientras que el coste de quitar los otros dos depende de si esa información puede reconstruirse a partir de las observaciones actuales. La esencia del bucle ReAct consiste en permitir que el modelo haga avanzar la tarea de manera continua mediante la incorporación constante de elementos a la trayectoria.
...
4. ★★ En el experimento de ablación, la ausencia de "retroalimentación de resultados de herramientas" hace que el Agente reintente hasta agotar su presupuesto de iteraciones. En un entorno de producción, ¿qué otras situaciones podrían llevar a un Agente a un bucle así? ¿Qué mecanismos de detección y terminación diseñarías?
